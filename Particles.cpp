                     ...
                     double Lifespan)
{
    ...
    if (ParticleID == (DWORD)-1)
        m_Frame = pOwner->m_Position.m_Frame;
    else
        m_Frame = pOwner->m_PartArray->m_pPartObjs[ParticleID]->m_Pos30.m_Frame;
    ...
    case 6:
        {
            ...
            float RA = Random::RollDice(-3.1415927, 3.1415927);
            float PO = Random::RollDice(-3.1415927, 3.1415927);
            float RB = cosf(PO);
    ...
    pPart->m_Scale = Vector(StartScale, StartScale, StartScale);
...
void Particle::Update(DWORD Info2C, BOOL Persistant, CPhysicsPart *pPartObj, Frame *pFrame)
{
    ...
    {
    case 1:
        pPartObj->m_Pos30.m_Frame.m_origin = pFrame->m_origin + m_58;
        break;
    case 2:
    case 12:
        pPartObj->m_Pos30.m_Frame.m_origin.x = (m_FrameTime * m_64.x) + (pFrame->m_origin.x + m_58.x);
        pPartObj->m_Pos30.m_Frame.m_origin.y = (m_FrameTime * m_64.y) + (pFrame->m_origin.y + m_58.y);
        pPartObj->m_Pos30.m_Frame.m_origin.z = (m_FrameTime * m_64.z) + (pFrame->m_origin.z + m_58.z);
        break;
    case 3:
    case 8:
    case 10:
        pPartObj->m_Pos30.m_Frame.m_origin.x = (m_FrameTime * m_FrameTime * (m_70.x / 2)) + (m_FrameTime * m_64.x) + (m_58.x + pFrame->m_origin.x);
        pPartObj->m_Pos30.m_Frame.m_origin.y = (m_FrameTime * m_FrameTime * (m_70.y / 2)) + (m_FrameTime * m_64.y) + (m_58.y + pFrame->m_origin.y);
        pPartObj->m_Pos30.m_Frame.m_origin.z = (m_FrameTime * m_FrameTime * (m_70.z / 2)) + (m_FrameTime * m_64.z) + (m_58.z + pFrame->m_origin.z);
        break;
    case 4:
    case 9:
    case 11:
        {
            ...
            pPartObj->m_Pos30.m_Frame = TempFrame;
            break;
        }
    case 5:
        pPartObj->m_Pos30.m_Frame.m_origin.x = pFrame->m_origin.x + m_58.x + (m_FrameTime * m_64.x) + (m_7C.x * cosf(m_70.x * m_FrameTime));
        pPartObj->m_Pos30.m_Frame.m_origin.y = pFrame->m_origin.y + m_58.y + (m_FrameTime * m_64.y) + (m_7C.y * sinf(m_70.y * m_FrameTime));
        pPartObj->m_Pos30.m_Frame.m_origin.z = pFrame->m_origin.z + m_58.z + (m_FrameTime * m_64.z) + (m_7C.z * cosf(m_70.z * m_FrameTime));
        break;
    case 7:
        pPartObj->m_Pos30.m_Frame.m_origin.x = pFrame->m_origin.x + m_58.x + (m_70.x * m_FrameTime * m_FrameTime) + (m_7C.x * cosf(m_64.x * m_FrameTime));
        pPartObj->m_Pos30.m_Frame.m_origin.y = pFrame->m_origin.y + m_58.y + (m_70.y * m_FrameTime * m_FrameTime) + (m_7C.y * cosf(m_64.x * m_FrameTime));
        pPartObj->m_Pos30.m_Frame.m_origin.z = pFrame->m_origin.z + m_58.z + (m_70.z * m_FrameTime * m_FrameTime) + (m_7C.z * cosf(m_64.x * m_FrameTime));
        break;
    case 6:
        pPartObj->m_Pos30.m_Frame.m_origin.x = pFrame->m_origin.x + m_58.x + (m_FrameTime * (m_64.x * m_7C.x) + (m_FrameTime * m_70.x));
        pPartObj->m_Pos30.m_Frame.m_origin.y = pFrame->m_origin.y + m_58.y + (m_FrameTime * (m_64.y * m_7C.y) + (m_FrameTime * m_70.y));
        pPartObj->m_Pos30.m_Frame.m_origin.z = pFrame->m_origin.z + m_58.z + (m_FrameTime * (m_64.z * m_7C.z) + (m_FrameTime * m_70.z));
        break;
    }
    ...
    pPartObj->m_Scale = Vector(CurrentScale, CurrentScale, CurrentScale);
    ...
    : m_Owner(pOwner), m_08(-1), m_EmitterID(0)
{
    ...
    m_90 = Time::GetTimeCurrent(); // Timer::m_timeCurrent;
    m_78 = Time::GetTimeCurrent(); // Timer::m_timeCurrent;
}
...
BOOL ParticleEmitter::SetInfo(ParticleEmitterInfo *pInfo)
{
    ...
    m_Origin = m_EmitterObj->m_Position.m_Frame.m_origin;
    m_Parts5C = m_EmitterObj->m_PartArray->m_pPartObjs;
    ...
    for (long i = 0; i < m_EmitterInfo->m_48; i++)
    {
        ...
        {
            UNFINISHED("m_Parts58[i]->NotifySurfaceTiles() call");
...
BOOL ParticleEmitter::ShouldEmitParticle()
{
    ...
    if (m_EmitterInfo->m_28 & 2)
        Offset = m_EmitterObj->m_Position.m_Frame.m_origin - m_Origin;
...
BOOL ParticleEmitter::UpdateParticles()
{
    ...
    if ((m_EmitterObj->m_ViewerDist > m_MaxDegradeDist))
    {
        if (!m_60)
        {
    ...
    else
    {
        ...
        for (unsigned long i = 0; i < (unsigned)m_EmitterInfo->m_48; i++)
        {
            if (m_Parts5C[i])
            {
                ...
                {
                    if (m_08 == (DWORD)-1)
                        pFrame = &m_Owner->m_Position.m_Frame;
                    else
                        pFrame = &m_Owner->m_PartArray->m_pPartObjs[m_08]->m_Pos30.m_Frame;
                }
                else
...
{
    m_68 = Time::GetTimeCurrent(); // Timer::m_timeCurrent
...
ParticleEmitterInfo::ParticleEmitterInfo() : DBObj(ObjCaches::ParticleEmitterInfos)
{
}
...
{
    if (pEmitterInfo)
        ObjCaches::ParticleEmitterInfos->Release(pEmitterInfo->m_Key);
}
...
BOOL ParticleEmitterInfo::ShouldEmitParticle(long a, long b, Vector *c, double d)
{
    ...
    if (m_28 & 1)
    {
        if ((Time::GetTimeCurrent() - d) > m_40) // Timer::m_timeCurrent
            return TRUE;
    }
    else
    if (m_28 & 2)
