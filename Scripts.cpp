...
BOOL ScriptManager::AddScriptInternal(PhysicsScript *pScript)
{
    ...
    if (m_pScriptsEnd)
        ...
        pNode->m_Time = Time::GetTimeCurrent(); // Time::m_timeCurrent
...
void ScriptManager::UpdateScripts()
{
    ...
    //  Timer::m_timeCurrent
    while (Time::GetTimeCurrent() >= m_EndTime)
    {
...
PhysicsScript::PhysicsScript() : DBObj(ObjCaches::PhysicsScripts), m_ScriptData(2)
{
    m_EndTime = 0;
}
...
{
    if (pScript)
        ObjCaches::PhysicsScripts->Release(pScript->m_Key);
}
...
PhysicsScriptTable::PhysicsScriptTable() : DBObj(ObjCaches::PhysicsScriptTables), m_TableData(4)
{
}
...
{
    if (pScriptTable)
        ObjCaches::PhysicsScripts->Release(pScriptTable->m_Key);
}
...
BOOL PhysicsScriptTable::UnPack(BYTE** ppData, ULONG iSize)
{
    ...
    for (long i = 0; i < EntryCount; i++)
    {
        ...
        PhysicsScriptTableData
            *pData = new PhysicsScriptTableData;
