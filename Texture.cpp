...
ImgTex::ImgTex() : DBObj(ObjCaches::Textures)
{
    m_dw18 = 0;
    m_dw1C = 0;
    m_dw20 = 0;
...
void ImgTex::Destroy()
{
    if (m_pPalette)
    {
        ObjCaches::Palettes->Release(m_pPalette->m_Key);
        m_pPalette = NULL;
    }
...
void ImgTex::releaseTexture(ImgTex *pTexture)
{
    ...
    {
        ObjCaches::Textures->Release(pTexture->m_Key);
    }
    else if (pTexture->m_Key)
    {
        if (!pTexture->Unlink())
        {
            if (texture_table.remove(pTexture->m_Key, &pTexture))
            {
                // Found, delete it.
                delete pTexture;
            }
...
ImgTex *ImgTex::makeCustomTexture(DWORD custom_texture_ID)
{
    ...
    if (pTexture) 
    {
        ...
        {
            pTexture->m_Key = custom_texture_ID;
            pTexture->Link();
...
BOOL ImgTex::Combine(ImgTex* pImgTex, Palette* pPalette, BOOL bUnknown, long Type)
{
    ...
    ImgTex *pDummy;
    bCached = tiled_image_table.lookup(pImgTex->m_Key, &pDummy);
...
BOOL ImgTex::UnPack(BYTE **ppData, ULONG iSize)
{
    ...
    ImgTex *dummy;
    BOOL bInCache = tiled_image_table.lookup(m_Key, &dummy);
...
ImgColor::ImgColor() : DBObj(ObjCaches::ImgColors)
{
   /*
    m_dw18 = 0;
    m_dw1C = 0;
   ...
   *ppData = *ppData + m_Length;
   ...
   SIZE szImage;
    ...
    // pStream.Release();
	 ...
	 /*
	 char sf[256];
	 sprintf(sf, "e:\\temp\\tex-0x%08X.bmp", m_Key);
	 D3DXSaveSurfaceToFileA(sf, D3DXIFF_BMP, pSurface, 0, 0);
	 */
	 /*
	 if (m_Key == 0x0600398D)
	 {
		 D3DXLoadSurfaceFromFileA(pSurface, NULL, 0, "e:\\temp\\tex-0x0600398D_NRM.bmp", 0, D3DX_DEFAULT, 0, 0);
	 }
	 */
   ...
   if (FAILED(g_pD3DDevice->CreateTexture(
      m_Width, m_Height, 1, 0, (D3DFORMAT)m_Format, D3DPOOL_DEFAULT, // (Flags & 0x1000)?D3DPOOL_DEFAULT:D3DPOOL_SYSTEMMEM,
        &m_pD3DTexture9, NULL)))
   {
      DEBUGOUT("Failed creating texture.\n");
      Destroy();
      return FALSE;
   }

   IDirect3DSurface8 *pSurface;
   if (FAILED(m_pD3DTexture9->GetSurfaceLevel(0, &pSurface)))
   {
      DEBUGOUT("Failed getting surface.\n");
      Destroy();
      return FALSE;
   }

   // D3DXLoadSurfaceFromMemory(pSurface, NULL, &rcDest, pMem, header.m_dwFormat, 2 * header.m_dwX, NULL, &rcSrc, D3DX_FILTER_NONE, 0xFF000000);
   // D3DXIMAGE_INFO info;
   // ::D3DXLoadSurfaceFromMemory(pSurface, NULL, &rcDest, m_Data, (D3DFORMAT)m_Format, m_Width * 2, NULL, &rcSrc, D3DX_FILTER_NONE, 0xFF000000);
